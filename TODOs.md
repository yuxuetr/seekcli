...
*来源：L7-7。依赖：三十七。设计：待补（`docs/architecture/L7-observability.md`）*
...
- [ ] **41.4 MCP 图像透传**（L2-9）：`[image content omitted]` 改为真的传过去。
    - [x] **已验证这条 wire 接受 `tool` 角色的多段内容**（2026-09-13 实测，
          模型答出了图里的两种颜色）——所以不被协议阻塞，只被我们自己的类型阻塞。
          坑：thinking 模式下 assistant 消息必须带回 `reasoning_content`。
    - [ ] ⏸ **剩下的是一个设计决定**：图像怎么从 `execute_with` 那个
          `Result<String>` 闭包里出来，同时不给「所有工具走同一条守门路径」
          开缝。三个候选与各自代价见设计 §4.6.4。
          **本轮刻意没做**——仓促选一个会造出这个代码库一直避免的那种缝。
...
- [ ] **41.6 eval 增补**：一条带图的任务，断言模型真的用到了图像内容。
