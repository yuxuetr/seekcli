...
- [ ] **38.2 默认冒烟集**（`basic.json`，3 个任务）。
      **默认必须是「跑一点」而不是「不跑」**——默认不跑等于这个功能不存在。
    - [ ] ⚠️ **只对 `skill` 提案跑**：`mcp` 要重启才连上、`task` 是给调度器的提示词，
          对它们跑 eval 是表演。闸门要**说清楚为什么不跑**，静默跳过会让用户
          以为通过了检查。详见设计 §4.7.1。
    - [ ] 衡量的是**回归**不是能力：「激活它之后，agent 在原本就会做的事情上
          是否变差了」。把它说成「验证 skill 有效」是过度宣称（§4.7.2）。
    - [ ] 基线**当场跑不缓存**：过期的基线比没有基线更糟，它会给出自信的错判。
