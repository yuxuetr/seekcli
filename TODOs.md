...
**何时重估**：需要同进程并发运行多个 Run 时（例如 eval 并行化，或并行子代理）。
在那之前，全局状态是**单进程单运行**这个事实的诚实表达，不是欠债。

##### 2026-09-17 补：并行子代理给出了第一个带数字的候选受益者

实测三个 `explore` 子代理（一轮内三个 `invoke_agent`）：

| | |
| --- | --- |
| 串行实际 | 9.8 + 9.3 + 6.9 = **26.0s** |
| 若并行 | max = **9.8s** |
| 收益 | **2.7×** |

阻塞点精确可指：`run_agent_loop(&mut self)` 与 `delegate_to_subagent(&mut self)`。
两个并发子代理需要两个 `&mut App`。只读工具能并行是因为 `Self::dispatch` 是
**静态函数**、只取共享引用——子代理走的是重入循环那条路。

子代理运行期真正需要可变的只有两样：`self.cost`（12 处）与 `self.tracer`（23 处）。
把它们换成共享可变（`Arc<Mutex<…>>`）之后并行就成立——这正是 48.2 RunContext
那条路的一个切片。

**仍然不做，理由是需求没出现过。** 今天所有真实运行里，模型**从未自发**扇出多个
...
推翻它的是一句质疑：**「如果是为了加速复杂任务，并行不是更好吗？」**

我用「模型从未自发扇出子代理」当作不需要的证据，这个推理有洞——**子代理当时是
串行的，扇出三个不比自己做快，所以模型不扇出可能恰恰是对当前实现的理性反应**。
把「没观察到」当成「不需要」，正是 CLAUDE.md「没有证据支持 ≠ 有证据反对」要挡住
的那一步。

而且工具并行替代不了子代理并行：前者表达不了「你去把 X 搞清楚，需要几步就几步」。

**实测 33s → 16s。** 设计与代价见
[L5 §4.7](docs/architecture/L5-composition.md)。三件事在并发下仍然成立：
运行预算（共享 `AtomicU64`，先预留再调用）、中断传播、审批互斥。
（原先的两项代价——子代理内部无 trace、`duration_ms` 记整批墙钟——
均已于阶段五十三消除。`duration_ms` 改为各记各的，
阶段五十三改为各记各的——见 L5 §4.7.3 的订正。）

##### ✅ 2026-09-17 修正（阶段五十三）—— 并行分支漏走了回合尾部

阶段五十二加的并发委派分支以自己的 `continue` 结束，于是**绕过了每个回合共同的
尾部**。被跳过的三件事：

| 跳过的 | 后果 |
| --- | --- |
| `plan_next = depth == 0 && turn_had_failure` | 并发子代理全部失败也**无法触发** Two-Stage 微规划。该分支在 `:1884` 计算了 `turn_had_failure`，然后**从未读取它** |
| `reminder_injector.observe(...)` | 同一批委派重复无数轮，死循环检测一次也没被喂过 |
| 顺序点 2 的 `flush_journal` | 「结果落盘后才回给模型」在这条路上不成立——事件要等下一轮的顺序点 1，而那在 `request_step` **之后** |

**编译器看不见**（提前 `continue` 完全合法），**行为测试也看不见**（每个分支单独看
都对）。这是「加了新路径，忘了它也欠公共尾部」这一类。

修法是**结构性的，不是往分支里补三行**：三条派发策略改写成
`if / else if / else`，共用唯一出口——没有第二个地方需要同步。

守护：`the_turn_loop_has_one_exit` 用 `include_str!` 抽出 `run_agent_loop` 的函数体，
断言其中没有 `continue;`（并先自检抽取器，防止空 body 让断言空过）。
反向验证过：插回 `continue` → 测试红并指出行号；移除 → 绿。

实跑验证：并发两个 `explore`，trace 里该回合的 `execute` span 带的是**共享尾部**
...
##### ✅ 2026-09-17 修正（阶段五十三）—— 撞到迭代上限的子代理不再一无所获

告知模型「同轮委派会并发」之后，第一次量 `ChildRun` 的结果就暴露了下一个问题：
**20 次历史委派里 4 次撞满迭代上限，而父级拿到的是 40 字节的占位符。**

根因在 `run_sub_agent`：`text` **只在模型停止调用工具时**被赋值。所以一个撞顶
时还在干活的子代理，15 次模型调用的全部发现被整个丢弃，父级只收到
`[Stopped at max iterations (15)]`。

修法是撞顶后**再发一次请求，工具留空**——它没法再开一轮调查，只能把已有的
说出来。只在迭代上限这一种结束方式上做：预算耗尽的不该再花，被中断的是
用户让它停。这一次调用同样走 `fetch_add` 预留，预算不够就退回占位符。

顺带修掉一个会让它白做的坑：`request_step` 原先无条件传 `Some(tools.to_vec())`，
空切片会变成 `"tools": []`——**与「不传」在 Rust 里看不出区别，在协议上是两回事**，
provider 会拒绝或误处理。`planning_phase` 一直是直接传 `None` 的，
现在 `request_step` 与它一致。有 spy provider 单测钉住，反向验证过。

实测同一个问题、同一个提示，撞顶的那个子代理返回从 **40 字节**变成
**6876 字节带 file:line 的发现**，还自己说明了为什么慢
（`--read-only` 拦了它的 shell 调用——那是测试参数造成的，不是产品问题）。

##### ✅ 2026-09-17 修正（阶段五十三）—— 子代理内部不再是盲区

阶段五十二把「子代理内部不产生 trace span」列为并发的**明确代价**，理由是
`Trace::begin` 需要 `&mut`、加锁不划算。加锁确实不划算，但**「所以什么都不记」
是错的结论**——它在同一天挡住了两次调查：「那次撞顶是不是死循环」与
「哪个子任务慢」。

第三条路一直在这个代码库里：`CostTracker::absorb`。子代理自己攒一棵树
（`Trace::child()`），回来由父级嫁接到那一轮的 `execute` 之下
（`Trace::absorb`）。**不需要锁，`run_sub_agent` 仍然是 `&self`。**
时钟在 `child()` 里就按父级基准钉死，所以并发子代理在树里重叠——那是事实。

关闭时全链路零成本：`child()` 继承 `enabled=false`，`begin` 是 no-op，
`absorb` 直接返回。有单测钉这条。

三条 `absorb` 测试，反向验证过（去掉 id 偏移 → 两条红）：子树保持自身嵌套并
落在嫁接点下、两棵子树互不串、关闭时吸收为空。

**第一次用它就结了一桩悬案。** 撞顶的那次 15 轮轨迹从不重复：

```
iter 0 [list_dir, glob]    iter 8  [run_shell, run_shell]
iter 1 [read_file×2]       iter 10 [run_shell, grep, read_file]
iter 2 [run_shell, glob]   iter 12 [run_shell, read_file]
iter 7 [grep, read_file]   iter 14 [run_shell, run_shell]
```

**不是死循环，是 `explore` 的 `max_iter = 15` 对这种两段式问题定低了。**
主循环那种按轨迹哈希、连续三次相同才干预的检测器，在这 15 轮上一次都不会触发
——所以「给子代理加死循环检测」这条待办，依据被这次测量推翻了。

同时看见：`run_shell` 每次约 800ms 且在子代理内部串行（两个 = 1610ms），
47 秒里约 16 秒花在 shell 上。

##### ✅ 2026-09-17 修正（阶段五十三）—— 每次 `run_shell` 都白付 800ms

子代理 trace 刚打开就看见了它：`run_shell` 的耗时是 **805 / 807 / 804 / 830ms**，
一条太平的线。命令本身不会这么齐——**那是个固定开销**。

`spawn_delayed_spinner` 开头是无条件的 `sleep(SPINNER_DELAY)`（800ms），
而命令结束后调用方 `spinner_task.await` **要等这个 sleep 睡完**。所以每一次
`run_shell` 都固定多付 800ms，`echo hi` 也不例外。

它藏了这么久，是因为**这个数字看起来像命令自己的开销**。只有把同一个工具
连续十几次的耗时排在一起，那条平线才刺眼——这正是子代理 trace 打开后的第一个
收获，而两小时前它还是个盲区。

修法是让等待可被中断（`tokio::select!` + `Notify`），而不是睡完再查标志。
用 `notify_one` 而非 `notify_waiters`：它在任务尚未进入 await 时会留下一个许可，
所以「命令比任务首次被轮询还快」也能唤醒。

| | 之前 | 之后 |
| --- | --- | --- |
| `echo hi` | 806ms | **< 10ms** |
| 真实运行里每次 `run_shell` 均摊 | ~805ms | **22ms** |
| 一次 `explore` 委派的 shell 死时间 | ~8s | ~0 |

两条测试，各守一侧，都反向验证过：快命令不得付延迟（改回无条件 sleep → 红，
报 806ms）；慢命令不得被提前打断、也不得在自己的一秒之上再付一次延迟。

顺带的证据：修完后同一个问题，两个子代理里有一个 **11 轮就完成了**，
此前两个都撞满 15 轮。

##### ❌ 2026-09-17 不做（阶段五十三）—— 调高 `explore` 的 `max_iter`

trace 打开后看到 6/6 撞顶，我当时把「15 定低了」写进了下一步。**测量之后撤销。**

`scripts/delegation-health.py` 扫剥离后的全部委派（22 次）：

| | |
| --- | --- |
| `explore` | 19 次，**迭代中位数 5**，5 次撞顶 |
| `general` | 3 次，中位数 2，0 次撞顶 |
| 撞顶率 | **23%**（阈值 33%） |

**典型委派只用 5 轮，15 是够的。** 撞顶集中在「一次问两件事」这种重负载，
而那三件事已经分别处理了：撞顶不再一无所获（salvage）、每次 shell 省回 800ms、
以及模型现在知道该把独立子任务拆成多个并发委派而不是塞给一个。
修完之后同一个问题里已经有子代理 **11 轮完成**。

调高上限是**用钱买余量**，而现在没有证据说需要买。

**重估条件可执行**：`python3 scripts/delegation-health.py` 退出码非零，
即撞顶率超过三分之一。反向验证过（4/5 撞顶的假数据 → 红；1/5 → 绿；
无委派时明确报「这本身也是一个发现，不是通过」）。

##### ⏸ 2026-09-17 待数据（阶段五十三）—— 剩下两条缺的是判据不是实现

这两条今天反复出现，但都**还写不出可证伪的判据**，所以先记下来而不是排期——
按本仓的 gap 闸门逻辑，不记就等于悄悄掉出计划。

**（一）轮次是全局 barrier —— 归入已有的 L5-5，不另立条目。**
依赖靠「放同一轮 / 放下一轮」表达，代价是一轮里最慢的那个决定整轮：
A、B 独立而 C 只依赖 A 时，C 仍要等 B。今天实测的量级：同一轮两个子代理
29.6s 与 78.1s，所以这种 barrier 最坏能吃掉约 48s。

**我一开始把它当成新问题记了，其实它是 ReAct 回合结构的固有属性**，
不是本实现的缺陷：模型要写下一轮的 prompt，就必须等这一轮结果全部回来。
要让 C 在 A 完成时立刻开始，就得离开「每轮重新决策」换成 DAG 执行器——
那正是 P3 表里 `L5-5 workflow / DAG 编排｜是另一个产品`。
**判据已经有了，不需要再等数据。**

**（二）委派质量反馈闭环 —— 一半已解，一半明确不做。**

*(a) prompt 自包含性 ✅ 已解。* 原先的失败形态是沉默的：模型写
「继续分析上面那个文件」，子代理看不见「上面」，于是猜，回来一段驴唇不对马嘴
的摘要，父代理当成有效结果继续推理，全程无人报错。

现在两个模板的系统提示都要求：指代无法解析时立即回
`[PROMPT INCOMPLETE]` 并说明缺什么，**不要猜**——
「一份自信的、关于错误对象的摘要，父代理无法把它和正确的区分开」。
父代理提示相应写明该标记的含义与处置（补上缺失内容重发，不要重复原 prompt，
也不要把拒绝当成发现）。

这是**一份跨两个文件的提示契约**，缺哪一半都比两半都没有更糟；
编译器看到的是两个不相关的字符串常量，所以用一条测试钉住两端，
反向验证过（删掉父代理那半 → 红）。

实跑：故意下发「继续分析上面提到的那个文件」，子代理如实拒绝，
父代理转述为拒绝并提出补上路径重发。

`scripts/delegation-health.py` 统计这个标记的出现次数——**机制与度量一起落地**，
否则下次还是只能凭印象。合成数据验证过 call_id 的连接。

*(b)「该委派而没委派」❌ 不做。* 至今没有诚实的代理指标。今天试过
「主上下文膨胀」，结论是 **offload 已经在工具层解决了膨胀**（剥离后工具输出
进主上下文最大 16.8KB、P90 3.6KB），那条路不通。脚本特意**不**去猜这件事：
编一个指标出来会被当成证据。

##### ✅ 2026-09-17 修正（阶段五十三）—— `/trace`：记了要看得见

子代理内部能记之后还剩一个洞：**记了也没有入口看**。只有
`~/.seekcli/traces/<uuid>.json`，而 run_id 只在 flush 时往 stderr 打一行。
我当天为了读它**手写了三遍一次性的 Python 渲染器**——那正是「有了第二个
消费者」的判据。

`/trace [run-id]`：不带参数看最近一次，带前缀看指定的。
「最近」按**修改时间**而不是文件名——run_id 是 v4 UUID，按名字排序会返回一个
随机的运行，而且看起来像是有意挑的。

渲染是选择性的而不是把 meta 倒出来：每个 span 一行，只展示携带答案的那几个
字段（工具清单 / 结束原因 / 调用数）。**JSON 读不出来的原因就是信号被埋了**——
判断「那次撞顶是不是死循环」要把十五个 `execute` 的工具列表并排看，
而在 pretty-print 的 JSON 里它们相隔几百行。

空目录时不只报「没有」，而是说明**追踪是 opt-in 且事后开不了**，要重跑加
`SEEKCLI_TRACE=1` —— 那是这个错误最可能的原因。

##### ✅ 2026-09-17 修正（阶段五十三）—— 追踪默认打开

上面那条「仍未解决」当天解决了。**追踪只在你已经知道会出事时才有用，
而调试恰恰发生在事后**——opt-in 的追踪等于「需要它的时候它不在」。
今天两次撞上：都得先发现异常，再重跑一次带 `SEEKCLI_TRACE=1`。

```toml
[trace]
enabled = true   # 默认
keep = 200       # 超出后按修改时间删最旧的；0 = 全留
```

- **`SEEKCLI_TRACE` 双向覆盖。** 旧语义是 `is_ok()`——**任何值都算开，
  所以 `SEEKCLI_TRACE=0` 会把它打开**。现在 `0`/`off`/`false`/空 关，其余开。
- **保留在 flush 之后做**，所以刚写的那份永远不是删除候选；并发的另一个进程
  写出的也是最新的，因此不需要锁。只删 `.json`——这个目录是我们的，
  但删掉不是我们写的东西不叫整理。
- **成功时不再打印路径。** 默认开了之后，每个回答底下都跟一行文件路径是噪音；
  `/trace` 就是发现入口。失败仍然出声——一份悄悄没写成的 trace，
  是你在需要它的时候才发现的。
- 实测单次 7–45KB，`keep = 200` 约几 MB。

四条测试：保留只留最新的（反向验证：把排序方向写反 → 幸存的是 a、b，
**即刚跑完的那次被删了**）、`keep = 0` 是全留而不是全删、非 `.json` 不动、
env 双向覆盖（反向验证：改回 `Ok(_) => true` → 红）。
实跑 `keep = 3` 跑五次，剩三份且最新那份在。

> **一次没验成的反向验证。** env 那条我第一次「验证」时测试是绿的——因为
> `cargo fmt` 把那行折成了三行，我的字符串替换根本没匹配上，改了个寂寞。
> 重做才看到它变红。**`cargo fmt` 之后再按原文锚定，是这条老坑的又一次复发。**

##### ✅ 2026-09-17 修正（阶段五十三）—— 阶段零的产出物，此前无处可放

路线图把「阶段零 一周真实使用 → **产出失败清单** → 校准下方所有 B 类」
画进了依赖图，而 49.2 / 49.3 也都写着「需真实失败样本」。

**但 seekcli 里没有任何记录一次失败的入口。** 直接叫人去试用，等于让他
凭记忆攒一周的失败清单——一周之后那份清单不会存在。这与今天其余的发现同类：
计划的前置条件在代码里不存在。

`/bad [why]` + `EventPayload::MarkedBad` + `scripts/failure-list.py`：

- **零摩擦是设计目标而不是修饰。** 发现回答不对的那一刻，多一步就会放弃记录
  ——和贴图只按一个键同一个理由。所以理由是**可选的**：逼人写理由，
  正是在最容易丢失数据的时刻加摩擦。
- **记为事件而不是另开一个文件**：一句话的抱怨离开它所指的轨迹几乎没有价值。
  事件坐在会话里，`seekcli --resume <id>` 能重放整轮，`/trace` 能看循环做了什么，
  所以那句话不必自己承载证据。
- **投影时不产生消息**（与 `ContextInjected` 同一待遇）。否则下一轮、以及此后
  每一次 `/resume`，模型都会去回答那句抱怨而不是任务。有单测钉住「在日志里、
  不在对话里」这两条，反向验证过（改成会投影 → 红）。
- **当场落盘**，不等回合结束：一条只存在内存里的抱怨，会被那个引发它的
  崩溃或 Ctrl-C 一起带走。

实跑：提问 → `/bad 回答太啰嗦，我只要一个数字` → `failure-list.py` 打印出
时间、会话、模型、当时的 prompt、原话，以及一行 `seekcli --resume fa803f38`。
...
| — | 子代理死循环检测 | **依据被 trace 推翻**：撞顶那次 15 轮轨迹从不重复，按轨迹哈希的检测器一次都不会触发 |
| — | 调高 `explore` 的 `max_iter` | **中位数只有 5 轮**，撞顶率 23% 且已有 salvage 兜底，见下方专条 |
