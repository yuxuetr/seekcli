...
**（一）轮次是全局 barrier。** 依赖靠「放同一轮 / 放下一轮」表达，代价是
一轮里最慢的那个决定整轮：A、B 独立而 C 只依赖 A 时，C 仍要等 B。
DAG 能表达，轮次不能。

*缺的数据*：真实任务里**静态依赖**（A 做完立刻就能写出 C 的 prompt）占多少。
若占多数，DAG 才真的更好；若 C 的 prompt 必须看过 A 的实际结果才写得出，
barrier 就不是损失而是必要。今天的日志里样本太少（7 次全委派回合）。

**（二）委派质量没有反馈闭环。** 「子代理 prompt 必须自包含」只是提示里的
一句话，没有任何验证；而「该委派而没委派」至今没有诚实的代理指标——
今天试过用「主上下文膨胀」来量，结论是**offload 已经在工具层解决了膨胀**
（剥离后工具输出进主上下文最大 16.8KB、P90 3.6KB），所以那条路不通。
`delegation-health.py` 特意**不**去猜这件事：编一个指标出来会被当成证据。
