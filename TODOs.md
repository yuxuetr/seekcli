...
- [ ] **41.2 `EventPayload` schema 演进**：按「模型可见 = 已记录」，
      图像进入请求就必须能从 `events.jsonl` 重建。
    - [ ] 事件日志有版本与迁移的先例（阶段二十六），沿用同一套做法。
    - [ ] 图像**不内联进日志**：blob 归属机制已经存在（`tools/offload.rs`），
          日志里存引用。否则一次截图会让 `events.jsonl` 膨胀数百 KB。
- [ ] **41.3 token 计数与成本口径复查**：16×16 的图令 prompt 从 31 涨到 224。
...
- [ ] **41.5 `/paste` 图像输入**（L6-6）：截图 → `Cmd+Shift+4` → `/paste`，
      **零路径管理**。macOS 用 osascript 读剪贴板；内部临时文件（如
      `/tmp/seekcli_paste.png`）对用户不可见，可以接受。
    - [ ] **不做**客户端扫描消息里的路径自动塞图——那才是 §1 排除的 `@xxx` 模式。
...
**本轮的第一前提**：五评核查 `~/.seekcli/` 得出——九层文档、351 测试、30 个 eval
任务、四平台发布**全部成立**，但**真实使用记录为零**。审计日志 767 条调用全部来自
测试夹具（`T/seekcli-loop-*`）与 eval testbed；最近一次真实交互会话停在 2026-08-23，
内容是 `先读 z.md，如果不存在就创建` 这类冒烟 prompt；MCP 零配置；launchd 零安装；
金融 skill 不存在；雅思三个 skill 是 6 行提示词壳；累计 proposal 唯一一个是
`user_lucky_number`（冒烟产物）。
...
- [ ] **42.1 事件日志名实不符**（L4-9，**缺陷级**）
    - [ ] 现状：`history.rs:8` 模块头自称 `events.jsonl append-only log <- the source
          of truth`，实际 `history.rs:83-90` 是 `fs::write(全部事件)` 整文件重写；
          热路径唯一调用点 `engine.rs:557`，**整轮结束才落一次**。
    - [ ] 改为增量 append；确立落盘顺序「先记待执行动作与审批结果 → 执行 →
          记结果 → 再交给模型」。
    - [ ] 关键记录写不下去时**暂停有副作用的执行**，而不是一边失去追踪一边继续改文件。
...
- [ ] **42.3 中断不进子代理**（L1-7，**缺陷级**）
    - [ ] 现状：`engine.rs:382`（流内）与 `engine.rs:962`（轮顶）两处中断检查都是
          `depth == 0` 门控。子代理跑起来后 Ctrl-C 不被它的循环看见——
          explore 15 轮 / general 20 轮可能干等很久。
    - [ ] `tools::shell::set_interrupt` 会杀掉正在跑的 shell，但循环继续迭代，
          表现为「命令被杀了但 agent 还在转」。
- [ ] **42.4 压缩阈值与模型窗口脱钩**（L4-10）
    - [ ] 现状：`compressor.rs:40` 的 150K 与 `compressor.rs:44` 的 `KEEP_TAIL = 8`
          都是全局常量，而 provider 由配置静态选择、支持三套 wire 协议。
    - [ ] 换个窗口小的模型，压缩要么永不触发要么触发太晚，**两种失败都静默**。
    - [ ] 阈值改为「配置窗口 × 比例」；触发与降级各输出一行（原则 §4「绝不静默」）。
...
- [ ] **43.1 搜索与取原文**（非代码）：`~/.seekcli/skills/web_research/`，
      含 `search.sh` 与 `fetch.sh`。形态对齐 `doc_parser`——脚本返回**文件路径**
      而非内容本身，避免 stdout 过大。
- [ ] **43.2 引用契约**（Rust 侧，真正属于本项目的部分）：
      区分**搜索摘要**与**读过的原文**；结论携带 `source + 获取时间 + 是否读了原文`；
      取不到原文时明确说明限制，不用摘要冒充证据。
- [ ] **43.3 不可信输入边界**（L3-6，**不得晚于 43.1 落地**）：网页与文档里出现的
      「忽略以上规则」「执行以下命令」不得获得系统指令或工具授权地位。
      现在零防护，而 43.1 之后会第一次真的从网上取内容进来。

**验收**：对照失败清单里的金融问题，回答携带可核的来源与获取时间；
一条 eval 用投毒网页断言注入不生效。
...
- [ ] **44.1** `references/` 的实际消费。现在 `enumerate_skill_assets`
      （`skills.rs:333`）只把 `scripts/` 与 `references/` **列成清单**附进
      system_prompt，模型得自己去读。若一周试用显示「列清单 + 模型自己 read_file」
      已够用，**本项降级或取消**。
- [ ] **44.2** Skill 版本与来源记录——为阶段四十九的对照评估准备，
      **没有版本就没法回滚**。
...
- [ ] **45.1** 三档，**全部是可编辑 markdown，不上向量库**：
      会话上下文（已有）／领域状态（学习进度、薄弱项、关注标的）／长期偏好。
- [ ] **45.2** 每条持久记忆带**来源 / 适用范围 / 更新时间 / 撤销方式**。
- [ ] **45.3** **模型在一次失败后写下的反思不自动升级为永久规则**——
      沿用已有的 proposal 闸门思路。
- [ ] **45.4** 按模式按需读取，避免金融讨论中的假设混进学习或日常问答。

**验收**：隔一周再开，雅思场景能接着上次的薄弱项继续，且不串味。
...
- [ ] **47.1** 事件携带 span id，**trace 从同一事件链派生**，
      不再维护第二份可能与事实不一致的数据。
- [ ] **47.2** ChildRun 关联父 Run 与委派调用（子代理耗时归属）。
- [ ] **47.3** 区分**运行结束原因**（正常 / 预算耗尽 / 取消 / 错误 / 待审批）
      与**任务验证状态**（通过 / 失败 / 未验证 / 不适用）。
      **模型说「修好了」只是正常结束，不等于通过验证。**

---

### 🟤 阶段四十八：`lib.rs` + RunContext + 共享预算

*⚠️ **收益待阶段零清单确认。若清单里没有一条指向它，就往后放。***
*四评已把同类项（L1-2 turn/step 模型）判为「收益只有概念更清晰」。*

- [ ] **48.1** 增加 `lib.rs`，**保持单 crate，不拆多 crate**。核心不直接打印终端。
- [ ] **48.2** RunContext：显式工作目录（现在 `tasks.rs` 与 `benchmark.rs`
      各自实现 `chdir` 手法）、配置与权限实例化。
- [ ] **48.3** 共享预算替代孤立 `max_iter`：总时长 / 模型调用次数 / 工具调用次数 /
      子代理并发与深度。**子代理从父运行继承，而不是每启动一个就凭空拿一份完整预算**
      （现状 explore 15、general 20，只有 `MAX_SUBAGENT_DEPTH=3` 作总闸）。

**验收**：REPL / headless / `--run-task` / eval 四个入口共用同一运行服务，
结束语义完全一致。
...
- [ ] **49.1** 真实失败案例转 eval 任务，**与合成夹具分开计数**。
- [ ] **49.2** 分场景指标——「变好了」在四个场景里含义不同：
      问答看引用是否支持结论；编程看验证是否通过、返工是否减少；
      金融看数据时效与口径、计算是否可复现（**不用短期涨跌给自己评分**）；
      学习看相同错误是否减少（**不用「回答更长」判断**）。
- [ ] **49.3** 保留集：不参与提案生成的任务，防止提案者同时掌握全部答案。
- [ ] **49.4** 门禁失效检查：缺测试资源时必须**显式跳过**，不得自动接受。
...
- [ ] **50.1** 只做一件事：把 Skill / MCP / Task 三类资产的**安装、启用、禁用、版本**
      收成同一套声明式装配，即已有 `~/.seekcli/proposals/{skill,mcp,task}` 的正向对偶。
- [ ] **50.2** 正在运行的 Run 固定使用一份**扩展版本快照**，升级下一次 Run 生效。
      比运行中热插拔更适合个人工具。
...
| 约束 | 改法 | 触发阶段 |
| --- | --- | --- |
| 「扩展需求由 MCP 承担」 | 收窄为「进程外扩展由 MCP，带脚本的方法论扩展由 Skill」 | 43 |
| 「跨会话语义记忆」排除 | **推翻**。原文为 coding CLI 写，学习 / 金融场景改变了前提 | 45 |
