...
- [ ] **47.4 注入的系统消息进事件日志**（L4-12，阶段四十五补记）
    - [ ] `workspace_rules` / 记忆注入都直接插进 `messages`，不回流日志，
          **无法从 `events.jsonl` 重建**——违反 design-principles §3
          「模型可见 = 已记录」。
    - [ ] **坑在投影语义**：记成 `SystemPrompt` 事件后，记忆一变就追加一条新的，
          而旧的按 append-only 不能撤回（与 skill 激活同理），prompt 会堆积
          历史快照。要么接受堆积，要么让投影对某类事件只取最后一条——
          后者是投影语义的实质变更，所以与 Run 身份统一一起做。
