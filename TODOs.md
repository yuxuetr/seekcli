...
**验收**：把 2026-04 ~ 05 的真实 prompt 重放一遍——`@tavily 苹果股票价格`、
`@web 智谱的股价现在是多少？`、`帮我规划雅思备考计划`——看能不能答出来
**且带得出来源与获取时间**。这批 prompt 有原始会话可查，不需要等阶段零产出。
另加一条 eval 用投毒网页断言注入不生效。
