...
而不是一句「以后观察到再说」（见 `scripts/tool-batch-shapes.py`、
`scripts/check-gap-coverage.py`）。

但这带来一个新的失败模式：**判据写歪了，你会得到一个精确的错误答案，
而它看起来比模糊的正确直觉更可信。**

阶段五十三的实例：为「混合工具批次分组」写的门，**提交当天就红了**，而且
两个方向都错——

| 错法 | 后果 |
| --- | --- |
| 数**机会**而不是**收益** | 在 `[grep, grep, run_shell]` 上误报。实测两个并发 grep 的 `execute` span 是 **19ms**，同一轮的模型调用是 **999ms** |
| 集合漏了 `invoke_agent` | 在 `[invoke_agent, invoke_agent, run_shell]` 上漏报——唯一值几秒钟的形状，它**结构上数不到** |

所以：

- **判据按收益定，不按可能性定。** 「可以并发」不等于「值得并发」，
  这个区别只有量一下才知道。先测一个数，再写判据。
- **判据要反向验证**，和测试同一个标准：造一个**该红**的输入确认它红，
  造一个**该绿**的确认它绿。上面那张四种批次形状的表就是这么来的。
  一条从没红过的门，和没有门是一回事。
- **先问数据是谁产生的。** 两类污染都真实发生过：
  - **已经不存在的代码**——剥离日之前的会话里 `read_file` 还带 `offset`
    参数、还有个 50KB 截断。拿它们下结论是在回答一个关于已删代码的问题。
  - **自己的测试流量**——那次触发误报的 4 个批次全部来自同一次 A/B 对照组。
    脚本现在按日期打印分布，某一天远超其他天就是在提醒你这一点。
