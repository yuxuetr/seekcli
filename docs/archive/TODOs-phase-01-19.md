# 归档：SeekCLI 进化路线图（阶段一 ~ 阶段十九）

> **本文件已归档，仅供回溯。** 当前活跃路线图见仓库根目录 [`TODOs.md`](../../TODOs.md)。
>
> 归档时间：2026-08-22。归档时状态：阶段一 ~ 阶段十九全部完成（阶段十 10.3 为主动推迟项）。
> 归档原因：完成对照 [`deepseek-harness`](../evaluation/2026-08-harness-gap-analysis.md) 的全面评估后，
> 路线图按「逐层补全」重新规划，从阶段二十起接续。
...
## ✅ 阶段十八：L8 循环层（reminders + todos MVP）
*目标：把 SeekCLI 从"纯反应式单会话 Harness"升级为具备最小自动化闭环的个人日用 Agent——
调度 → 执行 → 通知 → 持久化状态，全部复用既有 Harness 组件，零新增 Rust 工具实现。*
*评估来源：与用户关于 Harness Engineering vs Loop Engineering 的讨论——
L0-L7 是单次执行的 Harness 骨架，L8 是决定"何时再跑一次、状态如何跨调用留存"的外层 Loop。*

- [x] **18.1 `run_headless` 返回值扩展**
    - [x] `Result<u64>` → `Result<(String, u64)>`，同时带回最终文本与 API 调用数。
    - [x] `benchmark.rs` 唯一调用点同步解构更新，不影响报表逻辑。
- [x] **18.2 `src/tasks.rs`：headless 任务分发**
    - [x] `resolve_dir(&Config) -> Result<PathBuf>`：解析并创建 `~/.seekcli/tasks/`
          （或 `config.toml [tasks] dir` 覆盖），含 `digest/` 子目录。
    - [x] `run_task(&mut App, name)`：chdir 进 tasks 目录 → `run_headless(prompt)`
          → chdir 回原 cwd —— 与 `benchmark.rs` 的 testbed chdir 模式完全一致，
          借此让 `write_file` 的 cwd 白名单允许模型落盘 reminders.md/todos.md/digest。
    - [x] `reminders_prompt()`：硬编码中文 prompt 模板（非通用任务定义文件——
          stocks/learning 等按相同模式后续新增 match 分支即可）。
- [x] **18.3 CLI 入口**
    - [x] `Cli` 新增 `--run-task <TASK_NAME>`；`main()` 三路分发
          `bench` / `run_task` / 交互 REPL。
- [x] **18.4 状态文件格式约定**
    - [x] `reminders.md`：`- [ ]/[x] YYYY-MM-DD[ HH:MM] <text> [notified:...]`；
          `todos.md`：`- [ ]/[x] <text> [due:YYYY-MM-DD]`（可选）。
    - [x] 防重复通知：模型在触发 osascript 后立即 `edit_file` 追加/更新
          `[notified:<本次到期值>]`；下次运行比对该值是否仍等于当前到期值。
          每条提醒只弹一次系统通知，过期未完成靠 digest 持续兑现，不做反复骚扰式弹窗。
    - [x] 每次运行覆盖 `digest/<today>.md`——当天摘要快照，供交互式提醒引用。
- [x] **18.5 通知与终端提醒通道**
    - [x] macOS 通知：模型经 `run_shell` 调 `osascript -e 'display notification ...'`——
          未命中 `approval::is_dangerous`，无需交互审批，适配无 TTY 的 launchd 场景。
    - [x] `tasks::pending_digest_notice(&Config)`：REPL 启动时基于 digest 文件"今天
          是否存在 + 今天是否已提醒过"的日期 marker 判断，`App::run()` 顶部打印一行
          提醒；一天只提醒一次，不管后台跑了多少次 `--run-task`。
- [x] **18.6 launchd 落地示例**
    - [x] `examples/launchd/com.seekcli.reminders.plist`：`StartInterval=900`，
          注明 `WorkingDirectory`（`config.toml` 相对路径读取处）与
          `DEEPSEEK_API_KEY` 需在 `EnvironmentVariables` 显式设置（launchd 不继承 shell profile）。
    - [x] README 补充安装步骤（`launchctl load`），用户手动安装，不提供自动化 installer。
- [x] **18.7 config 扩展**
    - [x] `config.toml [tasks] dir`（可选，默认 `~/.seekcli/tasks`），`#[serde(default)]`
          保证旧配置兼容。

> ⚠️ **实战验证中发现的已知问题**（不在本阶段修复范围，已记录）：
> Two-Stage ReAct 的 micro trigger（阶段十三 13.1，工具失败后强制先做一次纯文本
> 规划）触发后，紧接着的下一轮"带工具"请求，DeepSeek 有一定概率把工具调用写成
> 字面文本（形如 `<｜tool▁calls▁begin｜>` 的伪 token）塞进普通输出内容里，而不是
> 走 API 的结构化 `tool_calls` 字段——`SEEKCLI_TRACE=1` 决策树能看到该轮
> `"tool_calls": 0`，模型却在文本里自称"已完成"（幻觉）。复现与 L8 无关，纯交互式
> "读一个不存在的文件，不存在就创建"即可稳定复现。本阶段用 prompt 级规避解决
> （`reminders_prompt` 第一步改为先 `list_dir` 探测文件是否存在，避免对不存在的
> 文件调用 `read_file` 触发失败恢复→Two-Stage 链路），**根因仍在 `engine.rs` 的
> Two-Stage 与 DeepSeek 工具调用协议交互处，留作独立后续修复项**。

**验收**：
`seekcli --run-task reminders` 首次运行 → 自动创建空 `reminders.md`/`todos.md` +
当日 `digest/*.md`；手写一条已到期 reminder 再跑一次 → 收到 macOS 通知且该行被
打上 `[notified:...]`；15 分钟内再跑一次 → 不重复弹通知；交互式 `seekcli` 启动
时若 digest 是新的 → 打印一行 `[Digest]` 提醒，同一天内再次启动不重复提醒。
以上全部经真实 API 调用（非 mock）手动跑通验证。

- [x] **18.8 Task + Skill 组合机制**（为后续 stocks/learning 等需要真实数据源的
      任务铺路）
    - [x] `run_headless(prompt, skill: Option<&Skill>)`：与 `commands::activate_skill`
          完全一致的方式注入 Skill 的 system prompt + `to_api_tools()`；
          `benchmark.rs` 调用点同步传 `None`。
    - [x] `tasks::task_spec(name) -> (prompt, skill_name: Option<&str>)`：每个具名
          任务自己决定要不要配一个 Skill（硬编码，非通用命名约定）；
          `load_named_skill` 找不到对应 Skill 时报错退出，不静默降级成裸跑。
    - [x] 用临时 `_smoketest` Skill + 临时 task 分支实测验证：headless 运行中
          Skill 的 system prompt 确实被激活（模型回复带上 Skill 要求的强制前缀），
          验证后已移除临时代码与临时 skill 文件。

---

## ✅ 阶段十九：Two-Stage ReAct 假工具调用根因修复
*目标：修复阶段十八记录的已知问题——Two-Stage 规划轮之后，模型有概率把工具调用
写成字面伪文本而不是真实 `tool_calls`，导致下一轮"确认已完成"却什么都没做。
根因定位到两处，均已修复并用真实 API 反复验证。*

- [x] **19.1 根因一：规划轮后消息以 `assistant` 结尾，轮次结构失范** —
      `engine.rs::planning_phase` 把纯文本规划结果追加成 assistant 消息后，
      紧接着的"带工具"请求发送时，消息列表最后一条已经是 assistant——每个其它
      轮次都以 `tool`/`user` 结尾，这个形状对聊天模型来说不常见（"继续你自己的
      话"而不是"轮到你决策了"）。
    - [x] 新增 `App::append_plan_with_bridge`：规划文本追加为 assistant 消息后，
          再追加一条 `[System] Proceed: call the tool(s) needed to execute the
          plan above now.` 的合成 user 消息，恢复 assistant→user→assistant 的
          正常轮转——与 `agent::reminders::ReminderInjector` 用合成 user 消息
          打断死循环是同一手法。
- [x] **19.2 根因二：规划轮的系统提示与"不许叙述,直接调用"的指令冲突** —
      `agent_system_prompt()` 里"Do not narrate 'I will now call X' -- just
      call it"这条规则是为正常轮次写的；规划轮故意不给工具 schema
      （`tools: None`），模型在"不许叙述、直接调用"和"这轮根本没工具可调"之间
      被迫选择，有时会把内部函数调用语法（形如 `<｜｜...｜｜tool_calls>`，
      全角竖线 U+FF5C 包裹的伪标签）当作叙述方式直接吐进 content——这不是
      Rust 侧解析 bug，`SEEKCLI_TRACE=1` 决策树能看到该轮 API 响应本身
      `tool_calls` 字段就是空的。
    - [x] 新增 `App::planning_only_directive()`：只追加到规划轮请求的**临时**
          消息副本上（不进永久历史，不占用/污染 prompt cache 静态前缀），
          明确告诉模型"这一轮没有工具可调，别写任何工具调用语法，用大白话
          说你的计划"。
    - [x] 概率性指令不够可靠（复现测试里仍有相当比例继续吐伪语法）——加了
          确定性兜底：`App::strip_fake_tool_syntax`，检测规划文本里的
          `｜｜`（全角竖线连续两个）标记，命中就从该处截断，只保留标记前的
          正常叙述部分，避免这段"看起来已经执行过"的伪文本被写进历史、
          误导下一轮真正带工具的请求以为工作已经做完。命中时打印一行
          `[Plan] discarded suspected fake tool-call syntax from plan text`
          可见提示。
- [x] **19.3 单测** — `engine.rs` 新增 4 个纯逻辑单测（不需要网络）：
      `plan_bridge_appends_assistant_then_user` / `plan_bridge_skips_empty_plan` /
      `planning_only_directive_is_system_and_forbids_tool_syntax` /
      `strip_fake_tool_syntax_truncates_at_marker` /
      `strip_fake_tool_syntax_leaves_clean_prose_untouched`。

**验收**：`cargo test`（87 通过）+ `cargo clippy --no-deps` 零警告 + `cargo fmt --check`
通过。真实 API 复现测试：修复前"读一个不存在的文件，不存在就创建"必现失败
（模型宣称完成但 `SEEKCLI_TRACE` 显示该轮 `tool_calls: 0`，文件也确实没被创建）；
修复后单文件场景 3/3 次、双文件并行场景 5/5 次全部实际创建成功（其中 5/5 次
console 都打印了 `[Plan] discarded suspected fake tool-call syntax`，证明模型仍在
频繁尝试假调用，是确定性兜底而非侥幸让结果保持正确）；`--run-task reminders`
复跑同样正常。
