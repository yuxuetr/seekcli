...
这条链由 `scripts/check-gap-coverage.py` 在 CI 与 pre-commit 中**强制**，双向校验：

- 评估里定义的每个缺口，必须被某个阶段承接，或列进 `TODOs.md` 的 P3「明确不排期」表；
- `TODOs.md` / 架构文档引用的每个编号，必须在评估里有定义，否则无法追溯到证据。

两个方向都有过真实漏网（L2-7 有设计无阶段、L6-5 有阶段无证据），所以是机械校验而非人工约定。
