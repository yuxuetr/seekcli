...
| 排除项 | 理由 |
| --- | --- |
| ~~跨会话语义记忆~~ | **阶段四十五推翻**，理由见 §2.1。收窄为：**不做向量检索 / 嵌入语义记忆**；分档的可编辑 Markdown 记忆已落地 |
...
### 2.1 为什么推翻「跨会话语义记忆」（阶段四十五）

原文的理由是「与 CLI 即时性目标背离；session 事件日志 + 工作区 PLAN.md 已覆盖」。
**这句话是对 coding CLI 写的，而它的前提被用户的真实场景改掉了。**

- 一个当天开始当天结束的任务，`PLAN.md` 确实够。
- 备考雅思、走完 CQF、跟踪一个仓位，是**以月计**的状态，
  而且这些对话**没有工作目录**——`PLAN.md` 无处安放。

推翻的是排除项，不是它背后的审慎。落地形态仍然刻意保守：

| 仍然不做 | 已落地 |
| --- | --- |
| 向量库 / 嵌入 / 语义召回 | 一个 scope 一个 Markdown 文件 |
| 自动抽取「用户画像」 | 显式 `memory` 工具写入 |
| 模型自行确立永久规则 | **偏好走提案闸门**，领域状态可直接写 |

**检索方式是「读你正在谈的那个主题的文件」。** 对单人工具这既够用又可检视；
嵌入买到的是模糊召回，代价是一份没人能读、能审、能手改的语料。

关键的结构判断：**「模型在一次失败后写下的反思不能自动升级为永久规则」
不是提示词里的一句请求，而是一条路由。** 领域状态（事实、可修正、作用域窄）
模型直接写；长期偏好（关于用户本人、作用于所有会话）只能经
`/propose accept memory <name>` 落地。刚失败过的模型，恰恰是最可能写下
「用户不理解条件期望」的模型，而那句话此后会出现在每一次会话里。
