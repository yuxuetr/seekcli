...
此前几项**不是取舍，是缺陷**——声明的边界与实际执行的边界不一致，
用户会按声明去信任它。现已修复：

| 项 | 此前 | 现在 |
| --- | --- | --- |
| shell 写路径 | 完全不检查，文件白名单只挡得住手滑 | 重定向或写动词指向绝对 / `~` 路径 → `Ask` |
| 复合命令 | 整行当一个字符串判定，`ls; rm -rf /` 由 `ls` 决定 | 按分隔符与命令替换拆分，逐条判定取最严 |
| 「只看不改」 | 无任何模式能表达 | `--read-only` / `/readonly` |
| 事后追溯 | 无 | `~/.seekcli/audit.jsonl` |

**Plan Mode 不在此列。** 初版评估把它记为「名不副实的写入限制」是误判：
它在本项目里意为**状态外部化**（阶段十五），提示词明确要求模型写
PLAN.md / TODO.md。给它加写入限制会破坏它所命名的功能。
dsh / Claude Code 意义上的 plan mode，在这里叫 `--read-only`。
