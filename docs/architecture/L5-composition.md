...
并发做完之后，两种失败仍然是**无声的**：父级拿到一份看起来正常的摘要，
而它既可能是空的，也可能是关于错误对象的。

#### 4.8.1 撞到迭代上限时把发现捞回来

`run_sub_agent` 里 `text` **只在模型停止调用工具时**被赋值。所以一个撞顶时
还在干活的子代理，15 次模型调用的发现被整个丢弃，父级只收到
`[Stopped at max iterations (15)]`。实测：20 次历史委派里 4 次如此。

撞顶后**再发一次请求、工具留空**——它没法再开一轮调查，只能把已有的说出来。
只对迭代上限这一种结束方式做：预算耗尽的不该再花，被中断的是用户让它停。
这次调用同样走 `fetch_add` 预留，预算不够就退回占位符。

实测同一个问题，撞顶的那个子代理返回从 **40 字节**变成 **6876 字节带
file:line 的发现**。

> 配套修掉一个会让它白做的坑：`request_step` 原先无条件传
> `Some(tools.to_vec())`，空切片会变成 `"tools": []`——**与「不传」在 Rust 里
> 看不出区别，在协议上是两回事**。`planning_phase` 一直直接传 `None`。

#### 4.8.2 `[PROMPT INCOMPLETE]`：无法解析的 prompt 要响亮地失败

原先的失败形态：模型写「继续分析上面那个文件」，子代理**看不见「上面」**，
于是猜，回来一段驴唇不对马嘴的摘要，父代理当成有效结果继续推理，全程无人报错。

两个模板的系统提示现在都要求：指代无法解析时立即回 `[PROMPT INCOMPLETE]`
并说明缺什么，**不要猜**——理由就写在提示里：*一份自信的、关于错误对象的摘要，
父代理无法把它和正确的区分开*。父代理提示写明该标记的含义与处置
（补上缺失内容重发，不要重复原 prompt，也不要把拒绝当成发现）。

**这是一份跨两个文件的提示契约，缺哪一半都比两半都没有更糟**——子代理拒绝而
父代理把拒绝读成发现，或者父代理等一个没人产出的标记。编译器看到的只是两个
不相关的字符串常量，所以用一条测试钉住两端
（`registry.rs::both_ends_of_the_incomplete_prompt_contract_exist`）。

`scripts/delegation-health.py` 统计该标记的出现次数——**机制与度量一起落地**。
少数几次是机制在工作，持续出现就是父代理的 prompt 有问题。
...
### 4.5.1 references/ 递归（阶段四十四）

**先澄清一个错误印象**：`references/` 本来就是通的。`read_file` 不在
`policy::is_mutating_with` 的名单里，所以 `ensure_within_cwd` 的工作区限制
不适用于它——模型读得到 `~/.seekcli/skills/<name>/references/` 下的文件。

**真实的缺口在递归。** `list_asset_entries` 原先只列当前目录，
而教材类语料的正常形状是 `chapter2/section3/`。实测：建一个带
`references/chapter2/ito.md` 的 skill，模型**确实找到了**那个文件——
用掉 `list_dir` ×2 + `read_file` **三次额外调用**去发现，
**而且只因为它想到了要找**。

想不到的时候没有任何提示。清单看不见那份材料，模型就用先验回答，
**外观上却像是有依据的**——这比读不到更糟。

改动三点：

- **递归，深度上限 3**。清单进的是每次请求的 prompt，所以边界是有的。
- **相对路径展示**（`chapter2/ito.md`），而不是每行重复一遍绝对路径。
- **条目上限 40，超出时明说还有几条并给出 `glob` / `grep` 的去处**。
  静默截断在这里是最坏的一种：模型会把清单当成语料的全部。

改后同一问题，模型直接 `read_file` 命中嵌套路径，首次尝试。
...
**工具并行表达不了「你去把 X 搞清楚，需要几步就几步」。** 只读工具能扇出，
但每个分支只是一次调用；独立的多步子问题只有子代理能表达，而子代理此前是串行的。

实测三个 `explore`：

| | |
| --- | --- |
| 串行 | 9.8 + 9.3 + 6.9 = **26.0s**，占该轮墙钟 79% |
| 并行后（实测） | 整轮 **33s → 16s** |

#### 4.7.1 阻塞点与解法

阻塞不是某个字段，是一条 `&mut self` 链：`run_agent_loop` →
`request_step` / `delegate_to_subagent` / `flush_journal`。两个并发子代理需要两个
`&mut App`。只读工具能并行是因为 `Self::dispatch` 是**静态函数**只取共享引用。

解法**不是**给 `cost` / `tracer` 加锁（那是 60 处调用点的改造），而是：

1. `request_step` 改为 `&self`——它本来就只有 `self.cost.record` 需要可变，
   而 usage 自阶段四十七起已经随 `Response` 返回，把记账挪给调用方即可。
2. 新增 `run_sub_agent(&self, …)`：同一套控制流，**去掉 depth-0 那一半**
   （压缩、Two-Stage 规划、日志落盘、技能激活、doom-loop 检测、嵌套委派——
   每一项本来就在 `if depth == 0` 后面）。
3. 每个子代理**自带** `CostTracker`，归队后由父级 `absorb` 合并。
   加法即合并，没有锁序问题。

**重复是有界的，而且是刻意的**：这个循环的特性面**冻结**。子代理需要「思考、
调工具、回答」以外的任何东西，都是重新审视子代理设计的理由，不是把这个函数养大的
理由。

#### 4.7.2 并发下仍然成立的三件事

- **运行预算**：共享一个 `AtomicU64`，**先 `fetch_add` 预留再调用**，
  所以三个并发子代理不会各自花掉最后一格。实测上限 4 时，三个子代理各跑 2 轮后
  同时 `budget_exhausted`，父运行随即停止，总计恰好 4 次。
- **中断**：子代理只 `load` 不 `swap`，父级仍看得见（阶段四十二的不变量未变）。
- **审批**：`confirm_interactive` 加了互斥锁——它写 stderr 再读 stdin，两处都
  不能同时来自两个地方，否则问题交错、答案错配，而那种 bug 没有堆栈可查。

#### 4.7.2b 哪些能并发，由模板契约决定

扇出的条件是**两个**：一轮内全是 `invoke_agent`，**且**每个目标模板都标了
`parallel_safe`。

第二个条件不是可选的。`general` 存在的目的就是改东西，而 `edit_file` 是
读-改-写——两个并发跑在同一个文件上会**交错成损坏**，而不只是冲突。
这与工具级并行早就有的 `is_parallel_readonly` 是同一条规则，只是此前在子代理这层
漏掉了。

| 模板 | `parallel_safe` | 理由 |
| --- | --- | --- |
| `explore` | ✅ | 存在的目的是看 |
| `general` | ❌ | 存在的目的是改；两个改同一文件是常态不是边角 |

**这个标记是声明的，不是推导的，而这个差距值得点名**：`explore` 带着
`run_shell`，它能写。契约和系统提示词都说它不该写，`--read-only` 下也确实拦得住，
但普通模式下 harness 不强制。所以这个标记表达的是**「打算用于并发」**，
比「不可能冲突」弱。

有测试守住：任何带 `write_file` / `edit_file` 的模板不得标 `parallel_safe`；
同时断言至少有一个标了，否则这个特性会变成永不触发的死代码。

**串行并没有被取代。** 三个 `general` 实测仍然依次执行（21s，三个文件都正确创建），
三个 `explore` 并发（16s）。依赖型任务本来也不需要这个开关——模型会把它们分到
不同轮次，先看到 A 的结果再决定 B。**一轮内发出三个，本身就是模型在断言它们互不
依赖。**

#### 4.7.3 明确的代价

> **2026-09-17 订正。** 这里原先写「子代理内部不再产生 trace span」，理由是
> `Trace::begin` 需要 `&mut`、为它加锁是笔差交易，而 `ChildRun` 事件已经够用。
> **加锁确实是差交易，但「所以什么都不记」是错的结论**——它在同一天挡住了两次
> 调查：「那次撞顶是不是死循环」和「哪个子任务慢」，都因为委派内部是盲区而答不了。
>
> 第三条路早就在这个代码库里：`CostTracker::absorb`。子代理**自己攒一棵树**
> （`Trace::child()`），回来由父级嫁接（`Trace::absorb`）。不需要锁，
> `run_sub_agent` 仍然是 `&self`。时钟在 `child()` 里就按父级的基准钉死，
> 所以并发子代理在树里是重叠的——那正是关于它们的事实。
>
> 第一次用它就得到了答案：撞顶的那次 15 轮轨迹**从不重复**
> （`[list_dir,glob]` → `[read_file,read_file]` → `[run_shell,glob]` → …），
> 所以**不是死循环**——主循环那种按轨迹哈希的检测器在这 15 轮上一次都不会触发。
>
> **同日再订正**：上面这句后面原本还跟着「是迭代上限定低了」。
> 那是从一次运行推出来的，当天的测量推翻了它：`scripts/delegation-health.py`
> 扫全部委派，`explore` 的迭代**中位数只有 5**，撞顶率 23%。
> 15 对典型委派够用，撞顶集中在「一次问两件事」这种重负载。
> **一次观察不是一个分布**，而当时我正好用它写了一条待办。

> **2026-09-17 订正。** 这里原先写的是「`duration_ms` 对并发批次记整批墙钟，
> 三条记录因此相同」，理由是「并发运行里没有哪一个因为单独跑而更快」。
> 那个理由站得住，结论不成立：**它把这个字段唯一能回答的问题销毁了**——
> 三条 16000ms 说不出哪个子任务慢，而「该拆哪个子任务」正是要看它的时候。
> 批次墙钟本来就是 `execute` span 自己的 `dur_ms`，记两遍换来的是丢掉分项。
>
> 现在每个子代理给自己计时。**它们重叠**这件事由上层的 span 表达：
> 子代理耗时之和远大于 span 的 `dur_ms`，差额就是并发省下的。
