...
### 4.7 eval 回灌提案闸门（L7-7）

> 七条件里第 5 条（可判定的评价信号）与第 6 条（选择压力）此前各自存在但**没有
> 连接**：`bench.rs` 与 `ProposalStore::accept` 之间没有任何调用关系。
> 本节把它们接上，**但接的面比路线图初稿窄得多**，理由在下面。

#### 4.7.1 只有 `skill` 提案能被 eval 衡量

路线图初稿写「`/propose accept` 前跑相关 eval 套件」，没有区分类型。查下去发现
另外两类根本测不到：

| 类型 | 能不能被 eval 衡量 |
| --- | --- |
| `skill` | ✅ 它改的是系统提示，**当场就会改变模型行为** |
| `mcp` | ❌ server 要**重启**才连上，本进程内跑多少 eval 都碰不到它 |
| `task` | ❌ `TASK.md` 是给调度器的提示词，不参与当前会话 |

对后两类跑 eval 是表演：花了钱和时间，却没有测到任何与该提案有关的东西。
**闸门要说清楚为什么不跑，而不是假装跑过了**——静默跳过会让用户以为
「通过了检查」。

#### 4.7.2 它衡量的是回归，不是能力

一个 skill 是领域专用的（`rust_backend`、`ielts_coach`），而 eval 套件是通用的。
所以这道闸门**不问「这个 skill 好不好」**——它问：

> **激活它之后，agent 在它原本就会做的事情上是否变差了？**

一段写坏的 skill 提示词（比如「永远先反问用户」）会让 agent 在所有通用任务上
退化。这是真实且值得拦的失败模式，而且**是可判定的**。把这道闸门说成
「验证 skill 有效」则是过度宣称。

#### 4.7.3 基线当场跑，不缓存

同一次调用里跑两遍：不带 skill 与带 skill。

缓存基线会更省钱，但**一份过期的基线比没有基线更糟**——模型换了、工具面变了、
提示词改了，它都不知道，却会给出一个自信的错误判决。

代价因此是透明的、且必须**事前告知**：约 2N 次 LLM 调用。冒烟集默认用
`basic.json`（3 个任务），一次接受大约 12 次调用、半分钟、几分钱。

#### 4.7.4 判决与逃生口

任一任务**由通过变失败**即拒绝接受，并**点名是哪几个**——与
[L1 §4.6](L1-engine.md) 同一条规则：拒绝必须可行动。

`--skip-eval` 是逃生口，但**必须打印跳过了什么**。一个悄悄跳过检查的逃生口
等于没有检查。

判决逻辑本身是纯函数（`before` × `after` → `Decision`），所以「故意引入回归
必须被挡下」这条可以用单测确定性地验证，不需要每次都烧一次真实调用。
