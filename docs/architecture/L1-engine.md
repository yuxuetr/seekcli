...
> 完成度 **95%**（阶段四十二、四十七、四十八后）｜ 缺口来源：[评估 §3 L1](../evaluation/2026-08-harness-gap-analysis.md#l1-引擎层--80最强的一层)
...
### 2.1 每个回合只有一个出口（阶段五十三）

派发一轮工具调用有三条策略（并发委派 / 并发只读 / 串行），它们只决定**怎么跑**；
跑完之后的收尾是三条路共同欠下的：结果落盘、喂死循环检测器、设置 Two-Stage
微触发。

阶段五十二加的并发委派分支以自己的 `continue` 结束，于是**这三件事在那条路上
从未执行**。最刺眼的是它在 `:1884` 计算了 `turn_had_failure` 然后**从未读取**——
并发子代理全部失败也无法触发规划。

**编译器看不见**（提前 `continue` 完全合法），**行为测试也看不见**（每个分支
单独看都对）。修法是结构性的而不是往分支里补三行：三条策略写成
`if / else if / else`，共用唯一出口，**没有第二个地方需要同步**。

`engine.rs::the_turn_loop_has_one_exit` 用 `include_str!` 抽出 `run_agent_loop`
的函数体，断言其中没有 `continue;`，并先自检抽取器——空 body 会让断言空过，
那正是这条测试要防的失败模式往上一层复制一遍。
